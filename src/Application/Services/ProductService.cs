...
namespace FulfillmentInventoryPlatform.Application.Services
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ProductResponseDto> UpdateAsync(int id, UpdateProductDto dto, CancellationToken ct = default)
        {
            ...
            // Only the product's *current* price changes here - orders created before
            // this update keep their own UnitPriceSnapshot untouched (see OrderItem).
        ...
        private static ProductResponseDto Map(Product p) => new(
            p.Id, p.Name, p.Description, p.IsActive, p.CategoryId, p.Category?.Name ?? string.Empty, p.Price);
    }
}
